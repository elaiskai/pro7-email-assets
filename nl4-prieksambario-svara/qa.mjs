...
try{for(const [name,width,nostyles,dark] of [['desktop',600,false,false],['mobile320',320,false,false],['mobile390',390,false,false],['mobile430',430,false,false],['no-head320',320,true,false],['dark390',390,false,true]]){
 ...
 await page.route('https://raw.githubusercontent.com/elaiskai/pro7-email-assets/main/nl4-prieksambario-svara/assets/*',async route=>{const file=route.request().url().split('/').pop();assert.ok(/^[a-z-]+\.(png|jpg)$/.test(file));await route.fulfill({path:new URL('assets/'+file,root).pathname})});
