...
for(const [src,dest,width] of [['hero-sos-kalkes-v3.png','hero-sos-kalkes-v3.jpg',1200],['amecid-sink-before.png','amecid-sink-before.jpg',650],['amecid-sink-after.png','amecid-sink-after.jpg',650],...['toilet','shower','tap'].flatMap(item=>['before','after'].map(state=>[`amecid-${item}-${state}-source.jpg`,`amecid-${item}-${state}.jpg`,650]))]){
...
`<tr><td><a href="${product}"><img src="assets/hero-sos-kalkes-v3.jpg" width="600" alt="SOS. Kalkės puola? AMECID čiaupams, dušo stiklui ir kalkių apnašoms. 10 % nuolaida prenumeratoriams." style="display:block;width:100%;max-width:600px;height:auto;border:0"></a></td></tr>`+
...
row(`<a href="${product}" style="display:block;background:#ffffff;border-radius:12px;padding:24px"><img src="assets/amecid-1l.png" width="118" alt="AMECID kalkių ir rūdžių valiklis, 1 l" style="display:block;width:118px;max-width:100%;height:auto;margin:0 auto;border:0"></a>`,'0 32px 20px','bgcolor="#f3f5ef"')+
