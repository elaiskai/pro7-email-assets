...
// Change to v4 and rebuild to restore the previous hero without reverting later copy edits.
const heroVersion='v5';
...
for(const [src,dest,width] of [[`hero-sos-kalkes-${heroVersion}.png`,`hero-sos-kalkes-${heroVersion}.jpg`,1200],['milda-review-original.png','milda-review.jpg',1080],['amecid-sink-before.png','amecid-sink-before.jpg',650],['amecid-sink-after.png','amecid-sink-after.jpg',650],...['toilet','shower','tap'].flatMap(item=>['before','after'].map(state=>[`amecid-${item}-${state}-source.jpg`,`amecid-${item}-${state}.jpg`,650]))]){
...
row(p('Pažiūrėkite, kaip šiuos paviršius pavyko išvalyti su <strong>AMECID</strong>. O apsipirkti šįkart galite su <strong>10 % nuolaida</strong>.'),'0 32px 24px')+
row(`<h1 class="heading" style="margin:0;font-size:28px;line-height:34px;font-weight:700">Jums 10 % nuolaida</h1>`,'24px 32px 10px','bgcolor="#e8f6f0"')+
row(p('Pirkdami įveskite kodą:'),'0 32px 18px','bgcolor="#e8f6f0"')
));
const coupon=wrap(table(row(`<p class="coupon-code" style="margin:0;padding:16px 12px;border:1px dashed #56826a;text-align:center;font-family:Montserrat,Arial,sans-serif;font-size:28px;line-height:36px;letter-spacing:3px;font-weight:700">KALKES</p><p class="offer-dates" style="margin:14px 0 0;font-size:13px;line-height:21px;text-align:center">Nuolaida galioja nuo rugsėjo 15 d.<br>iki rugsėjo 20 d. imtinai.</p>`,'0 32px 24px','bgcolor="#e8f6f0"')));
...
row(`<h2 class="heading" style="margin:0;font-size:28px;line-height:34px;font-weight:700">Mildos atsiliepimas</h2>`,'6px 32px 18px')+
row(`<a href="${product}"><img class="review-img" src="assets/milda-review.jpg" width="536" alt="Mildos 5 žvaigždučių atsiliepimas apie AMECID: Neatsidžiaugiu balta spalva. Ačiū už tokį produktą!" style="display:block;width:100%;max-width:536px;height:auto;border:0;border-radius:8px"></a>`,'0 32px 16px')+
row(`<p style="margin:0;font-size:15px;line-height:24px">Neatsidžiaugiu balta spalva. Ačiū už tokį produktą!</p><p class="muted" style="margin:7px 0 0;font-size:12px;line-height:18px;color:#65776c">Milda, atsiliepimo ištrauka</p>`,'0 32px 30px')+
...
row(`<p style="margin:0;font-size:28px;line-height:38px;font-weight:700"><del class="muted" style="font-size:18px;font-weight:400;color:#65776c;text-decoration:line-through;white-space:nowrap">32,21 €</del>&nbsp; <span class="discount-price" style="white-space:nowrap">28,99 €</span></p>${p('Su 10 % nuolaidos kodu <strong>KALKES</strong>.','font-size:12px;line-height:20px;margin-top:4px')}${p('Galioja nuo rugsėjo 15 d. iki rugsėjo 20 d. imtinai.','font-size:12px;line-height:20px;margin-top:3px')}`,'0 32px 16px','bgcolor="#f3f5ef"')+
...
// The shared KALKES code is now included. No native unique-code block is needed.
...
if(commit){
  ...
  const heroOnly=`<table role="presentation" width="100%" align="center" cellpadding="0" cellspacing="0" border="0" style="width:100%;max-width:600px;margin:0 auto;table-layout:fixed"><tr><td style="padding:0"><a href="${product}"><img src="assets/hero-sos-kalkes-${heroVersion}.jpg" width="600" alt="SOS! Kalkės puola! AMECID ir 10 % nuolaida prenumeratoriams." style="display:block;width:100%;max-width:600px;height:auto;border:0"></a></td></tr></table>`;
  for(const [name,content] of [['omnisend.html',body],['hero-omnisend.html',heroOnly],['omnisend-top.html',upper+coupon],['omnisend-bottom.html',lower]]){
...
console.log('Built index.html with shared KALKES code, September 15 through 20 validity, and client testimonial.');
